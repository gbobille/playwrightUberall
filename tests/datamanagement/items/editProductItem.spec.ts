...
test.describe("Create Product Item", { tag: '@dm_regression'}, () => {
    ...
    test(`Verify it is possible to create a Product Item @Production`, async ({ page }) => {
        ...
        await test.step(`Then user updates the new people item`, async () => {
            ...
            await itemsPage.clickEditButton("Automated Product Item Identifier");
        });

        await test.step(`And user edit all the fields for the new product item`, async () => {
            await itemsPage.identifierText.fill("Edited Product Item Identifier")
            await itemsPage.nameText.fill("Edited Product Item Name")
            await itemsPage.descriptionText.fill("Edited Product Item Description")
            await itemsPage.urlText.fill("https://www.uberall.com")
            await itemsPage.videoUrlText.fill("https://www.youtube.com/watch?v=EngW7tLk6R8")
        ...
        await test.step(`Then changes are persistent`, async () => {
            await expect(itemsPage.recentlyCreatedItem("Edited Product Item Identifier")).toBeVisible();
            await itemsPage.clickDeleteButton("Edited Product Item Identifier");
