...
test.describe("Create Custom Item", { tag: '@dm_regression'}, () => {
    ...
    test(`Verify it is possible to create a Custom Item @Production`, async ({ page }) => {
        ...
        await test.step(`Then user updates the new custom item`, async () => {
            await expect(itemsPage.recentlyCreatedItem("Automated Custom Item Identifier")).toBeVisible();
            await itemsPage.clickEditButton("Automated Custom Item Identifier");
        });

        await test.step(`And user edits the field for the created custom item`, async () => {
            await itemsPage.identifierText.fill("Edited Custom Item Identifier")
            await itemsPage.nameText.fill("Edited Custom Item Name")
            await itemsPage.descriptionText.fill("Edited Custom Item Description")
            await itemsPage.urlText.fill("https://www.uberall.com")
            await itemsPage.videoUrlText.fill("https://www.youtube.com/watch?v=EngW7tLk6R8")
        ...
        await test.step(`Then changes are persistent`, async () => {
            await expect(itemsPage.recentlyCreatedItem("Edited Custom Item Identifier")).toBeVisible();
            await itemsPage.clickDeleteButton("Edited Custom Item Identifier");
