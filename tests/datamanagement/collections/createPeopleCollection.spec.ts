...
test.describe("Locations Collection - People", { tag: '@dm_regression' }, () => {
    ...
    test(`Verify it is possible to create a People Collection`, async ({ page }) => {
        ...
        await test.step(`When user selects the option to create a new People collection`, async () => {
            const collectionType = "people";
            await collectionsPage.collectionTypeSelector(collectionType);
        });
        await test.step(`And user enters all the fields for the People collection`, async () => {
            await collectionsPage.nameText.fill("Automated People Collection Name");
            await collectionsPage.identifierText.fill("Automated People Collection" + Identifier);
            await collectionsPage.descriptionText.fill("Automated People Collection Description")
        });
        ...
        await test.step(`Then changes are persistent`, async () => {
            ...
            await collectionsPage.recentlyCreatedCollection("Automated People Collection" + Identifier).waitFor({ state: 'visible' });
            // Assert that the collection name is visible
            await expect(collectionsPage.recentlyCreatedCollection("Automated People Collection" + Identifier)).toBeVisible();
            // Click delete and confirm
            await collectionsPage.clickDeleteButton("Automated People Collection" + Identifier);
