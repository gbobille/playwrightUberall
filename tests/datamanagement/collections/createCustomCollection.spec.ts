...
test.describe("Locations Collection - Custom", { tag: '@dm_regression' }, () => {
    ...
    test(`Verify it is possible to create a Custom Collection`, async ({ page }) => {
        ...
        await test.step(`When user selects the option to create a new Custom collection`, async () => {
            const collectionType = "custom";
            await collectionsPage.collectionTypeSelector(collectionType);
        });
        await test.step(`And user enters all the fields for the Custom collection`, async () => {
            await collectionsPage.nameText.fill("Automated Custom Collection Name");
            await collectionsPage.identifierText.fill("Automated Custom Collection" + Identifier);
            await collectionsPage.descriptionText.fill("Automated Custom Collection Description")
        });
        ...
        await test.step(`Then changes are persistent`, async () => {
            ...
            await collectionsPage.recentlyCreatedCollection("Automated Custom Collection" + Identifier).waitFor({ state: 'visible' });
            // Assert that the collection identifier is visible
            await expect(collectionsPage.recentlyCreatedCollection("Automated Custom Collection" + Identifier)).toBeVisible();
            // Click delete and confirm
            await collectionsPage.clickDeleteButton("Automated Custom Collection" + Identifier);
