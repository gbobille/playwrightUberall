...
test.describe("Locations Collection - Menu", { tag: '@dm_regression' }, () => {
    ...
    test(`Verify it is possible to create a Menu Collection @Production`, async ({ page }) => {
        ...
        await test.step(`When user selects the option to create a new Menu collection`, async () => {
            const collectionType = "menu";
            await collectionsPage.collectionTypeSelector(collectionType);
        });
        await test.step(`And user enters all the fields for the menu collection`, async () => {
            await collectionsPage.nameText.fill("Automated Menu Collection Name");
            await collectionsPage.identifierText.fill("Automated Menu Collection" + Identifier);
            await collectionsPage.descriptionText.fill("Automated Menu Collection Description")
            await collectionsPage.urlText.fill("https://www.uberall.com")
            await collectionsPage.selectDropdownMenuCollection()
        });
        ...
        await test.step(`And user creates a new section`, async () => {
            ...
            await collectionsPage.sectionTitle.fill("Automated Section Collection");
        ...
        await test.step(`Then changes are persistent`, async () => {
            ...
            await collectionsPage.recentlyCreatedCollection("Automated Menu Collection" + Identifier).waitFor({ state: 'visible' });
            // Assert that the collection identifier is visible
            await expect(collectionsPage.recentlyCreatedCollection("Automated Menu Collection" + Identifier)).toBeVisible();
            // Click delete and confirm
            await collectionsPage.clickDeleteButton("Automated Menu Collection"+ Identifier);
