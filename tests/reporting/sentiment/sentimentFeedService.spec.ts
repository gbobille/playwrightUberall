...
test.describe('sentiment-feed-service tests @sentimentMicros', async () => {
    const apiUtils = new BaseCall();

    let baseURL = ''
    const env = process.env.UBERALL_ENVIRONMENT
    if (env == 'dev' || env == 'develop' || env == 'development') {
        baseURL = 'http://sentiment-feed-service.k8s.eu-central-1.development.uberall.com/'
    }
    else if (env == 'sandbox') {
        baseURL = 'https://sentiment-feed-service.k8s.eu-central-1.sandbox.uberall.com'
    }
    else if (env == 'prod' || env == 'production' || env == 'app') {
        baseURL = 'https://sentiment-feed-service.k8s.eu-central-1.production.uberall.com'
    }

    test('Verify Version in Prod Matches Version in DEV Environment for Sentiment Feed Service', async () => {
        const devBaseURL = 'http://sentiment-feed-service.k8s.eu-central-1.development.uberall.com/';
        const prodBaseURL = 'https://sentiment-feed-service.k8s.eu-central-1.production.uberall.com';
